...
def getMinNumMoves(blocks: List[int]) -> int:
    idxMax, maxValue = 0, blocks[0]
    idxMin, minValue = 0, blocks[0]
    for i, value in enumerate(blocks):
        if value > maxValue:
            idxMax, maxValue = i, value
        if value < minValue:
            idxMin, minValue = i, value
    if idxMin < idxMax:
        return idxMin + len(blocks) - idxMax - 1
    else:
        return idxMin + len(blocks) - idxMax - 2
