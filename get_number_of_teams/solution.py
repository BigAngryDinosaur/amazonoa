...
def getNumTeams(cost: List[int], min_cost: int, max_cost: int) -> int:
    ...
    pairs = 0
    n = len(cost)
    idx1, idx2_start, idx2_end = 0, n - 1, n - 1

    while idx1 < idx2_end:
        if idx1 < idx2_start and cost[idx1] + cost[idx2_start] >= min_cost:
            idx2_start -= 1
        elif cost[idx1] + cost[idx2_end] > max_cost:
            idx2_end -= 1
        else:
            pairs += idx2_end - idx2_start
            idx1 += 1
            idx2_start = max(idx2_start, idx1 + 1)
